...
ifile = open('/home/veror/Desktop/Uni/Semantic Web Technology/Final Project/esco-files/v1.0.0_1/occupations_en.csv', 'r')
...
outfile = open('esco_occupation.ttl', 'w')
...
for row in reader:
	if rownum == 0: # if it's the first row, then ignore it, move on to the next one.
		...
		c = row
		split_alt = c[4].splitlines()
		split_hidden = c[5].splitlines()
		if len(split_alt) != 0:
